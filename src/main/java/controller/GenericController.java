...
import com.example.laboratorio11.dto.DtoBase;
import com.example.laboratorio11.exception.InternalErrorException;
import com.example.laboratorio11.model.ModelBase;
...
public abstract class GenericController<E extends ModelBase, D extends DtoBase<E>> {
    ...
    protected void deleteElement(@PathVariable("id") @NotNull Long id) {
        getPromotionService().deleteById(id);
    }

    @GetMapping(value = "/model/{id}")
    public E findModelById(@PathVariable Long id) {
        return (E) getPromotionService().findById(id);
    }

    @GetMapping("/{id}")
    protected D getById(@PathVariable("id") @NotNull Long id) {
        return toDto((E) (getPromotionService().findById(id)));
    }

    @GetMapping
    protected List<D> getAll() {
        return toDto(getPromotionService().findAll());
    }

    protected List<D> findAll(String filter) {
        return toDto((getPromotionService().findAll(filter)));
    }

    @PostMapping
    protected D save(@RequestBody D element) {
        return toDto((E) getPromotionService().save(toModel(element)));
    }
    ...
    protected D patch(@PathVariable("id") @NotNull Long id, HttpServletRequest request) {
        E domain = (E) (getPromotionService().findById(id));
        E updatedDomain;
        D dto;
        try {
        ...
        return toDto((E) getPromotionService().patch(dto, updatedDomain));
    }

    protected abstract GenericService getPromotionService();
    ...
    public ResponseEntity uploadImage(@RequestParam("file") MultipartFile[] uploadingFiles,
        ...
        for (MultipartFile uploadedFile : uploadingFiles) {
            ...
            getPromotionService().saveImage(id, uploadedFile.getInputStream());
        }
